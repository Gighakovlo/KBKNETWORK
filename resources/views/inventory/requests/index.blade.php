...
<head>
    ...
    <title>IT Ticketing System - KBK</title>
...
<body class="text-slate-300 font-sans h-screen flex flex-col overflow-hidden">
    ...
    <nav class="p-6 bg-slate-900/80 backdrop-blur-md border-b border-slate-800 flex justify-between items-center z-20 shadow-xl">
        <div class="flex items-center gap-6">
            ...
            <div>
                <h1 class="text-2xl font-black text-white uppercase tracking-widest drop-shadow-md">IT Ticketing System</h1>
                <p class="text-fuchsia-400 text-xs mt-1 font-bold tracking-wide uppercase">Asset & Network Requests</p>
            </div>
        </div>
    ...
    <main class="flex-grow p-8 overflow-y-auto custom-scrollbar">
        <div class="max-w-7xl mx-auto space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="glass-panel p-6 rounded-2xl border-t-4 border-amber-500 relative overflow-hidden">
                    ...
                    <p class="text-3xl font-black text-amber-400">{{ $pendingCount }}</p>
                </div>
                <div class="glass-panel p-6 rounded-2xl border-t-4 border-emerald-500 relative overflow-hidden">
                    ...
                    <p class="text-3xl font-black text-emerald-400">{{ $completedCount }}</p>
                </div>
            </div>
            ...
            <div class="glass-panel rounded-2xl overflow-hidden shadow-2xl border border-slate-700/50">
                <div class="p-4 border-b border-slate-700 flex justify-between items-center bg-slate-900/50">
                    ...
                    <div class="relative group w-64">
                        <input type="text" id="searchInput" placeholder="Cari Nama / Departemen..." class="w-full bg-slate-900 border border-slate-700 text-white px-4 py-2 rounded-xl text-sm focus:border-fuchsia-500 focus:outline-none transition-all shadow-inner">
                    ...
                    <table class="w-full text-left text-sm whitespace-nowrap">
                        <thead class="text-slate-300 uppercase tracking-widest text-[10px] border-b-2 border-slate-600 bg-slate-900">
                            <tr>
                                ...
                                <th class="p-4 font-bold text-right">Aksi Eksekusi</th>
                            </tr>
                        </thead>
                        <tbody id="tableBody" class="divide-y divide-slate-700/80">
                            @forelse($requests as $req)
                                <tr class="hover:bg-slate-800/40 transition">
                                    <td class="p-4 text-xs font-mono text-slate-400">{{ \Carbon\Carbon::parse($req->created_at)->format('d M Y H:i') }}</td>
                                    <td class="p-4">
                                    ...
                                    <td class="p-4 text-right space-x-2">
                                        ...
                                        @endif
                                        <button onclick="confirmDelete('{{ $req->id }}')" class="text-slate-500 hover:text-red-400 font-bold px-3 py-1 transition text-xs">Hapus</button>
                                    </td>
                                </tr>
                            @empty
                ...
                <div class="p-4 border-t border-slate-800">{{ $requests->links() }}</div>
            </div>
        </div>
    </main>
